...
# 빈 데이터프레임 생성
CountryAccidentService2 = pd.DataFrame()

# 요청주소 입력
url = 'http://apis.data.go.kr/1262000/CountryAccidentService2/CountryAccidentService2'
...
CountryAccidentService2 = pd.concat([CountryAccidentService2, pd.DataFrame(request.json()['data'])]) # 제이슨 형태로 변환 후, 'data'만 뽑아서 데이터프레임화
CountryAccidentService2.drop_duplicates() # 중복제거
...
# 테이블 비우는 함수 실행
vacate('countryaccidentservice2')

# 값을 넣는 함수 실행
insert('countryaccidentservice2', CountryAccidentService2)
