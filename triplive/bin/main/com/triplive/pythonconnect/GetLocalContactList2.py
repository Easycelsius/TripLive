...
# 빈 데이터프레임 생성
getLocalContactList2 = pd.DataFrame()
...
getLocalContactList2 = pd.concat([getLocalContactList2, pd.DataFrame(request.json()['data'])]) # 제이슨 형태로 변환 후, 'data'만 뽑아서 데이터프레임화
getLocalContactList2.drop_duplicates() # 중복제거
...
# 테이블 비우는 함수 실행
vacate('getlocalcontactlist2')

# 값을 넣는 함수 실행
insert('getlocalcontactlist2', getLocalContactList2)
