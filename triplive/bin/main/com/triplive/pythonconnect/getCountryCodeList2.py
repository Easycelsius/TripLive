...
# 빈 데이터프레임 생성
getCountryCodeList2 = pd.DataFrame()
...
getCountryCodeList2 = pd.concat([getCountryCodeList2, pd.DataFrame(request.json()['data'])]) # 제이슨 형태로 변환 후, 'data'만 뽑아서 데이터프레임화
getCountryCodeList2.drop_duplicates() # 중복제거
getCountryCodeList2=getCountryCodeList2[getCountryCodeList2['iso_num']!=None]#null값 제거
getCountryCodeList2=getCountryCodeList2[getCountryCodeList2['iso_num']!='   ']#빈값 제거
...
# 테이블 비우는 함수 실행
vacate('getcountrycodelist2')

# 값을 넣는 함수 실행
insert('getcountrycodelist2', getCountryCodeList2)
