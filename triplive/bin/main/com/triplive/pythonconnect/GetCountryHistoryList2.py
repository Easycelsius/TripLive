...
# 빈 데이터프레임 생성
getCountryHistoryList2 = pd.DataFrame()
...
getCountryHistoryList2 = pd.concat([getCountryHistoryList2, pd.DataFrame(request.json()['data'])]) # 제이슨 형태로 변환 후, 'data'만 뽑아서 데이터프레임화
getCountryHistoryList2.drop_duplicates() # 중복제거
...
# 테이블 비우는 함수 실행
vacate('getcountryhistorylist2')

# 값을 넣는 함수 실행
insert('getcountryhistorylist2', getCountryHistoryList2)
