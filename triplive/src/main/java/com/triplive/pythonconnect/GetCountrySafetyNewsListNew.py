...
# 빈 데이터프레임 생성
getCountrySafetyNewsListNew = pd.DataFrame()
...
getCountrySafetyNewsListNew = pd.concat([getCountrySafetyNewsListNew, pd.DataFrame(request.json()['data'])]) # 제이슨 형태로 변환 후, 'data'만 뽑아서 데이터프레임화
getCountrySafetyNewsListNew.drop_duplicates() # 중복제거
...
# 테이블 비우는 함수 실행
vacate('getcountrysafetynewslistnew')

# 값을 넣는 함수 실행
insert('getcountrysafetynewslistnew', getCountrySafetyNewsListNew)
